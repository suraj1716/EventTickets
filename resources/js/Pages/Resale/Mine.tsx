// resources/js/Pages/Resale/Mine.tsx
//
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          {listings.data.length === 0 ? (
            <div className="border border-dashed border-[#26232E] rounded-2xl py-16 text-center">
              <h3 className="font-bold">No resale listings yet</h3>
              <p className="text-sm text-[#9C97A8] mt-2">
                List a ticket for resale from its ticket page.
              </p>
            </div>
          ) : (
              ...
              {listings.data.map((listing) => (
                <div key={listing.id} className="rounded-xl border border-[#26232E] bg-[#15141B] p-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span
                      className={`shrink-0 text-[10px] uppercase tracking-wide px-2 py-1 rounded-full border ${STATUS_STYLE[listing.status]}`}
                    >
                      {listing.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#26232E]">
                    ...
                    {listing.status === 'active' && (
                      <button
                        ...
                        className="text-xs text-[#6B6775] hover:text-red-400"
                      >
                        Cancel listing
                      </button>
                    )}
                    ...
                    {listing.status === 'sold' && (
                      <p className="text-xs text-[#9C97A8]">
                        ...
                        {listing.seller_paid_out ? (
                          ...
                          <span className="text-amber-400">Payout pending</span>
                        )}
                      </p>
                    )}
                  </div>
